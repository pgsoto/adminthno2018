...
class Barrios extends CI_Controller
{
    ...
    private $nombre = 'Barrios';
    private $modulo = 72, $modulo_imagenes = 73;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/tuciudad/barrios/';
        ...
        if (count($this->ws->listar($this->modulo)) == 0) {
            $data = array();
            $data['bar_estado'] = 1;
            $data['bar_url'] = slug($this->nombre);
            $data['bar_nombre'] = $this->nombre;
    ...
    public function agregar()
    {
        ...
        #js
        $this->layout->js('/js/sistema/tuciudad/barrios/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "bar_codigo = " . $codigo);
            if ($result) {
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galbar_seccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
                redirect('/tuciudad/barrios/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array($this->nombre => "/tuciudad/barrios/", $result->nombre => "/"));
        }

        #view
        $this->layout->view('barrios/add', $data);
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['bar_estado'] = 1;
                    $data['bar_url'] = slug($this->input->post('nombre'));
                    $data['bar_nombre'] = $this->input->post('nombre');
                    $data['bar_descripcion'] = $this->input->post('descripcion');

                    if ($this->input->post('ruta_interna_2')) {
                        $data['bar_imagen_ruta_interna'] = $this->input->post('ruta_interna_2');
                        $data['bar_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'bar_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galbar_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galbar_imagen_ruta_grande'] = $aux;
                                        $data2['galbar_seccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galbar_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galbar_imagen_ruta_grande'] = $aux;
                                        $data2['galbar_seccion'] = $codigo->bar_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->bar_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "bar_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galbar_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galbar_codigo = $codigo");
                }
            } elseif ($this->input->post('tipo') == 2) {
                if ($modelo = $this->ws->obtener($this->modulo, "bar_codigo = $codigo")) {
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna);
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande);
                    $data['bar_imagen_ruta_interna'] = '';
                    $data['bar_imagen_ruta_grande'] = '';
                    $this->ws->actualizar($this->modulo, $data, "bar_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
