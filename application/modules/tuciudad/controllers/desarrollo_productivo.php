...
class Desarrollo_productivo extends CI_Controller
{

    private $nombre = 'Desarrollo productivo';
    private $modulo = 56, $modulo_imagenes = 58;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/tuciudad/desarrollo_productivo/';
        ...
        if (count($this->ws->listar($this->modulo)) == 0) {
            $data = array();
            $data['depro_estado'] = 1;
            $data['depro_url'] = slug($this->nombre);
            $data['depro_nombre'] = $this->nombre;
    ...
    public function agregar()
    {
        ...
        #js
        $this->layout->js('/js/sistema/tuciudad/desarrollo_productivo/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "depro_codigo = " . $codigo);
            if ($result) {
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galdepro_seccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
        ...
        #view
        $this->layout->view('desarrollo_productivo/add', $data);
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    if ($this->input->post('ruta_interna_2')) {
                        $data['depro_imagen_ruta_interna'] = $this->input->post('ruta_interna_2');
                        $data['depro_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'depro_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galdepro_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galdepro_imagen_ruta_grande'] = $aux;
                                        $data2['galdepro_seccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galdepro_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galdepro_imagen_ruta_grande'] = $aux;
                                        $data2['galdepro_seccion'] = $codigo->depro_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->depro_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "depro_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galdepro_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galdepro_codigo = $codigo");
                }
            } elseif ($this->input->post('tipo') == 2) {
                if ($modelo = $this->ws->obtener($this->modulo, "depro_codigo = $codigo")) {
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna);
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande);
                    $data['depro_imagen_ruta_interna'] = '';
                    $data['depro_imagen_ruta_grande'] = '';
                    $this->ws->actualizar($this->modulo, $data, "depro_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
