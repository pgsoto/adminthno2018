...
class Subsecciones_das extends CI_Controller
{

    private $nombre = 'Subsecciones DAS';
    private $modulo = 37, $modulo_imagenes = 38, $modulo_seccion = 35;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/municipio/das/subsecciones/';
    ...
    public function index()
    {
        ...
        #js
        $this->layout->js('/js/sistema/municipio/das/subsecciones/index.js');
        ...
        $config['base_url'] = '/municipio/das/subsecciones/' . $seccion . '/';
        $config['per_page'] = 20;
        ...
        #contenido
        $this->ws->order("subdas_orden ASC");
        $this->ws->limit($config['per_page'], ($config['per_page'] * $pagina));
        $data["result"] = $this->ws->listar($this->modulo, $where);
        $data['pagination'] = $this->pagination->create_links();

        $seccion = $this->ws->obtener($this->modulo_seccion, "das_codigo = " . $seccion);

        #Nav
        $this->layout->nav(array('DAS' => '/municipio/das/', $this->nombre => '/'));

        #view
        $this->layout->view('das/subsecciones/index', $data);
    }
    ...
    public function agregar($seccion = false, $codigo = false)
    {
        ...
        #js
        $this->layout->js('/js/sistema/municipio/das/subsecciones/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "subdas_codigo = " . $codigo);
            if ($result) {
                $result->mapa_coor = explode(",", $result->mapa);
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galsubdas_subseccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
                redirect('/municipio/das/subsecciones/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array("DAS" => "/municipio/das/", $this->nombre => "/municipio/das/subsecciones/" . $seccion->codigo . "/", "Editar " . $result->nombre => "/"));
        } else {
            $data['titulo'] = 'Agregar ' . $this->nombre;
            $this->layout->title('Agregar ' . $this->nombre);
            $this->layout->nav(array("DAS" => "/municipio/das/", $this->nombre => "/municipio/das/subsecciones/" . $seccion->codigo . "/", "Agregar " . $this->nombre => "/"));
        }

        #view
        $this->layout->view('das/subsecciones/add', $data);
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['subdas_estado'] = $this->input->post('estado');
                    $data['subdas_url'] = slug($this->input->post('nombre'));
                    ...
                    if ($this->input->post('ruta_interna_2')) {
                        ...
                        $data['subdas_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    if ($this->input->post("mapa"))
                        $data['subdas_mapa'] = str_replace(array("(", ")", " "), "", $this->input->post("mapa"));

                    $data['subdas_seccion'] = $this->input->post('seccion');

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'subdas_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galsubdas_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galsubdas_imagen_ruta_grande'] = $aux;
                                        $data2['galsubdas_subseccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galsubdas_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galsubdas_imagen_ruta_grande'] = $aux;
                                        $data2['galsubdas_subseccion'] = $codigo->subdas_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->subdas_codigo, "seccion" => $this->input->post('seccion')));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "subdas_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galsubdas_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galsubdas_codigo = $codigo");
                }
            } elseif ($this->input->post('tipo') == 2) {
                if ($modelo = $this->ws->obtener($this->modulo, "subdas_codigo = $codigo")) {
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna);
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande);
                    $data['subdas_imagen_ruta_interna'] = '';
                    $data['subdas_imagen_ruta_grande'] = '';
                    $this->ws->actualizar($this->modulo, $data, "subdas_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
