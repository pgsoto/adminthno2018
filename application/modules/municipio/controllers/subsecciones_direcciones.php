        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    if ($codigo > 0) {
                        if ($update = $this->ws->actualizar($this->modulo, $data, 'subdir_codigo = ' . $codigo)) {
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galsubdir_cimagen_ruta_interna'] = $internas[$k];
                                        $data2['galsubdir_cimagen_ruta_grande'] = $aux;
                                        $data2['galsubdir_cdireccion'] = $codigo->subdir_codigo;
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            if ($modelo = $this->ws->obtener($this->modulo, "subdir_codigo = $codigo")) {
                ...
                $this->ws->actualizar($this->modulo, array("subdir_imagen_ruta_interna" => ""), "subdir_codigo = $codigo");

                $this->ws->actualizar($this->modulo, array("subdir_imagen_ruta_grande" => ""), "subdir_codigo = $codigo");
            }
        }
