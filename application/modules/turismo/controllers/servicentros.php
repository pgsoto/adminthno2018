...
class Servicentros extends CI_Controller
{

    private $nombre = 'Servicentros';
    private $modulo = 100, $modulo_imagenes = 101;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/turismo/servicentros/';
        ...
        if (count($this->ws->listar($this->modulo)) == 0) {
            $data = array();
            $data['serv_estado'] = 1;
            $data['serv_url'] = slug($this->nombre);
            $data['serv_nombre'] = $this->nombre;
    ...
    public function agregar()
    {
        ...
        #js
        $this->layout->js('/js/sistema/turismo/servicentros/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "serv_codigo = " . $codigo);
            if ($result) {
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galserv_seccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
                redirect('/turismo/servicentros/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array($this->nombre => "/turismo/servicentros/", $result->nombre => "/"));
        }

        #view
        $this->layout->view('servicentros/add', $data);
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['serv_estado'] = 1;
                    $data['serv_url'] = slug($this->input->post('nombre'));
                    ...
                    if ($this->input->post('ruta_interna_2')) {
                        $data['serv_imagen_ruta_interna'] = $this->input->post('ruta_interna_2');
                        $data['serv_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'serv_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galserv_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galserv_imagen_ruta_grande'] = $aux;
                                        $data2['galserv_seccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galserv_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galserv_imagen_ruta_grande'] = $aux;
                                        $data2['galserv_seccion'] = $codigo->serv_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->serv_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "serv_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galserv_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galserv_codigo = $codigo");
                }
            } elseif ($this->input->post('tipo') == 2) {
                if ($modelo = $this->ws->obtener($this->modulo, "serv_codigo = $codigo")) {
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna);
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande);
                    $data['serv_imagen_ruta_interna'] = '';
                    $data['serv_imagen_ruta_grande'] = '';
                    $this->ws->actualizar($this->modulo, $data, "serv_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
