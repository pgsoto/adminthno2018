...
class Comercio extends CI_Controller
{
    ...
    private $nombre = 'Comercio';
    private $modulo = 94, $modulo_imagenes = 95;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/turismo/comercio/';
        ...
        if (count($this->ws->listar($this->modulo)) == 0) {
            $data = array();
            $data['com_estado'] = 1;
            $data['com_url'] = slug($this->nombre);
            $data['com_nombre'] = $this->nombre;
    ...
    public function agregar()
    {
        ...
        #js
        $this->layout->js('/js/sistema/turismo/comercio/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "com_codigo = " . $codigo);
            if ($result) {
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galcom_seccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
                redirect('/turismo/comercio/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array($this->nombre => "/turismo/comercio/", $result->nombre => "/"));
        }

        #view
        $this->layout->view('comercio/add', $data);
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['com_estado'] = 1;
                    $data['com_url'] = slug($this->input->post('nombre'));
                    $data['com_nombre'] = $this->input->post('nombre');
                    $data['com_descripcion'] = $this->input->post('descripcion');

                    if ($this->input->post('ruta_interna_2')) {
                        $data['com_imagen_ruta_interna'] = $this->input->post('ruta_interna_2');
                        $data['com_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'com_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galcom_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galcom_imagen_ruta_grande'] = $aux;
                                        $data2['galcom_seccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galcom_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galcom_imagen_ruta_grande'] = $aux;
                                        $data2['galcom_seccion'] = $codigo->com_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->com_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "com_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galcom_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galcom_codigo = $codigo");
                }
            } elseif ($this->input->post('tipo') == 2) {
                if ($modelo = $this->ws->obtener($this->modulo, "com_codigo = $codigo")) {
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_interna);
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande))
                        unlink($_SERVER['DOCUMENT_ROOT'] . $modelo->imagen_ruta_grande);
                    $data['com_imagen_ruta_interna'] = '';
                    $data['com_imagen_ruta_grande'] = '';
                    $this->ws->actualizar($this->modulo, $data, "com_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
