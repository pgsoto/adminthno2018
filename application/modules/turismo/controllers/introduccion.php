...
class Introduccion extends CI_Controller
{
    ...
    private $nombre = 'Introducción';
    private $modulo = 103, $modulo_imagenes = 104;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/turismo/introduccion/';
        ...
        if (count($this->ws->listar($this->modulo)) == 0) {
            $data = array();
            $data['int_estado'] = 1;
            $data['int_url'] = slug($this->nombre);
            $data['int_nombre'] = $this->nombre;
    ...
    public function agregar()
    {
        ...
        #js
        $this->layout->js('/js/sistema/turismo/introduccion/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "int_codigo = " . $codigo);
            if ($result) {
                $result->imagenes = $this->ws->listar($this->modulo_imagenes, "galint_seccion = " . $codigo);
            }
            #print_array($result);
            if (!$result) {
                redirect('/turismo/introduccion/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array($this->nombre => "/turismo/introduccion/", $result->nombre => "/"));
        }

        #view
        $this->layout->view('introduccion/add', $data);
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['int_estado'] = 1;
                    $data['int_url'] = slug($this->input->post('nombre'));
                    $data['int_nombre'] = $this->input->post('nombre');
                    $data['int_descripcion'] = $this->input->post('descripcion');

                    if ($this->input->post('ruta_interna_2')) {
                        $data['int_imagen_ruta_interna'] = $this->input->post('ruta_interna_2');
                        $data['int_imagen_ruta_grande'] = $this->input->post('ruta_grande_2');
                    }

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'int_codigo = ' . $codigo)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galint_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galint_imagen_ruta_grande'] = $aux;
                                        $data2['galint_seccion'] = $codigo;
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            ...
                            if ($grandes) {
                                foreach ($grandes as $k => $aux) {
                                    if ($aux) {
                                        $data2['galint_imagen_ruta_interna'] = $internas[$k];
                                        $data2['galint_imagen_ruta_grande'] = $aux;
                                        $data2['galint_seccion'] = $codigo->int_codigo;
                            ...
                            echo json_encode(array("result" => true, "codigo" => $codigo->int_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "int_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            ...
            if ($this->input->post('tipo') == 1) {
                if ($modelo = $this->ws->obtener($this->modulo_imagenes, "galint_codigo = $codigo")) {
                    ...
                    $this->ws->eliminar($this->modulo_imagenes, "galint_codigo = $codigo");
                }
            }
        }
        echo json_encode(array("result" => true));
