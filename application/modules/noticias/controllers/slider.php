...
class Slider extends CI_Controller
{
    ...
    private $nombre = 'Slider Noticias';
    private $modulo = 18;
    public $img;
    ...
    function __construct()
    {
        ...
        $this->img->upload_dir = '/imagenes/modulos/noticias/slider/';
    ...
    public function index()
    {
        ...
        #js
        $this->layout->js('/js/sistema/noticias/slider/index.js');

        $where = $and = "";
        $url = "";

        $where = "slin_visible = 1";
        $and = " and ";
        ...
        $config['base_url'] = '/noticias/slider/';
        $config['per_page'] = 20;
    ...
    {
        #js
        $this->layout->js('/js/sistema/noticias/slider/agregar.js');
        ...
        if ($codigo && is_numeric($codigo)) {
            $result = $this->ws->obtener($this->modulo, "slin_codigo = " . $codigo);
            #print_array($result);
            if (!$result) {
                redirect('/noticias/slider/');
            } else {
                $data['result'] = $result;
            }
        }
        ...
        if (isset($result)) {
            ...
            $this->layout->nav(array($this->nombre => "/noticias/slider/", "Editar " . $result->nombre => "/"));
        } else {
            $data['titulo'] = 'Agregar ' . $this->nombre;
            $this->layout->title('Agregar ' . $this->nombre);
            $this->layout->nav(array($this->nombre => "/noticias/slider/", "Agregar " . $this->nombre => "/"));
        }
        ...
        if ($this->input->post()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                try {
                    ...
                    $data['slin_estado'] = $this->input->post('estado');
                    $data['slin_url'] = slug($this->input->post('nombre'));
                    $data['slin_nombre'] = $this->input->post('nombre');
                    $data['slin_orden'] = $this->input->post('orden');

                    if ($this->input->post('ruta_interna_1'))
                        $data['slin_imagen_ruta_interna'] = $this->input->post('ruta_interna_1');

                    if ($this->input->post('ruta_grande_1'))
                        $data['slin_imagen_ruta_grande'] = $this->input->post('ruta_grande_1');

                    # Si es una actualización el código es mayor a 0 ya que 0 es el valor predeterminado
                    if ($codigo > 0) {
                        if ($this->ws->actualizar($this->modulo, $data, 'slin_codigo = ' . $codigo)) {
                        ...
                        if ($codigo = $this->ws->insertar($this->modulo, $data)) {
                            echo json_encode(array("result" => true, "codigo" => $codigo->slin_codigo));
                            exit;
                        } else {
                            echo json_encode(array("result" => false, "msg" => "Ha ocurrido un error inesperado. Por favor, inténtelo nuevamente."));
                            exit;
    ...
    {
        try {
            $this->ws->eliminar($this->modulo, "slin_codigo = {$this->input->post('codigo')}");
    ...
    public function eliminar_imagen()
    {
        ...
        if ($codigo = $this->input->post('codigo')) {
            if ($modelo = $this->ws->obtener($this->modulo, "slin_codigo = $codigo")) {
                ...
                $this->ws->actualizar($this->modulo, array("slin_imagen_ruta_interna" => ""), "slin_codigo = $codigo");

                $this->ws->actualizar($this->modulo, array("slin_imagen_ruta_grande" => ""), "slin_codigo = $codigo");
            }
        }
